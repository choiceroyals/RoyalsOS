...
export async function listRoyalAssets(
  query?: string,
  options?: { source?: RoyalAssetSource; status?: "active" | "trash" | "all" }
): Promise<RoyalAssetRecord[]> {
  const assets = await readIndex();

  const status = options?.status ?? "active";
  const sorted = assets
    .filter((asset) => status === "all" || (asset.status ?? "active") === status)
    .filter((asset) => !options?.source || asset.source === options.source)
...
export async function moveRoyalAssetToTrash(id: string): Promise<RoyalAssetRecord | null> {
  const assets = await readIndex();
  let updated: RoyalAssetRecord | null = null;
  const next = assets.map((asset) => {
    if (asset.id !== id) return asset;
    updated = { ...asset, status: "trash", deletedAt: new Date().toISOString() };
    return updated;
  });
  if (updated) await writeIndex(next);
  return updated;
}

export async function restoreRoyalAsset(id: string): Promise<RoyalAssetRecord | null> {
  const assets = await readIndex();
  let updated: RoyalAssetRecord | null = null;
  const next = assets.map((asset) => {
    if (asset.id !== id) return asset;
    updated = { ...asset, status: "active", deletedAt: null };
    return updated;
  });
  if (updated) await writeIndex(next);
  return updated;
...
export async function permanentlyDeleteRoyalAsset(id: string): Promise<boolean> {
  const assets = await readIndex();
  const target = assets.find((asset) => asset.id === id);
  if (!target) return false;
  const absolute = path.join(process.cwd(), "public", target.relativePath.replace(/^\//, ""));
  try { await fs.unlink(absolute); } catch {}
  await writeIndex(assets.filter((asset) => asset.id !== id));
  return true;
}
